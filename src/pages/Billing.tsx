...
import { fetchRates, convert, SUPPORTED_CURRENCIES, ALL_CURRENCY_CODES, ZERO_DECIMAL, type RateMap } from '../lib/fxRates'

// ── money formatter ───────────────────────────────────────────────────────────
const money = (n: number | null | undefined, currency = 'USD') =>
  n == null ? '—' : new Intl.NumberFormat('en-US', { style: 'currency', currency, maximumFractionDigits: ZERO_DECIMAL.has(currency) ? 0 : 2 }).format(n)
const fmtDate = (s: string | null | undefined) =>
  !s ? '—' : new Date(s).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })

// ── FX display context ────────────────────────────────────────────────────────
type FmtAmt = (n: number | null | undefined, nativeCurrency?: string) => string
type FxCtx = { fmtAmt: FmtAmt; displayCurrency: string }
const FxContext = createContext<FxCtx>({
  fmtAmt: (n, c = 'USD') => money(n, c),
  displayCurrency: 'USD',
})
const useFmt = () => useContext(FxContext)
...
// ── currency selector ─────────────────────────────────────────────────────────
function CurrencySelector({ value, onChange, accent }: { value: string; onChange: (c: string) => void; accent: string }) {
  const { t } = useT()
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 11.5, color: C.muted, letterSpacing: '.06em', textTransform: 'uppercase', marginBottom: 8 }}>{t('bill.viewIn')}</div>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {SUPPORTED_CURRENCIES.map(({ code }) => (
          <button key={code} onClick={() => onChange(code)}
            style={{
              fontSize: 12, fontWeight: 700, fontFamily: 'DM Mono,monospace',
              padding: '5px 11px', borderRadius: 999, cursor: 'pointer', lineHeight: 1,
              color: value === code ? C.navy900 : C.muted,
              background: value === code ? accent : 'transparent',
              border: `1px solid ${value === code ? accent : C.navy700}`,
            }}>
            {code}
          </button>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <select value={currency} onChange={e => setCurrency(e.target.value)} disabled={busy} style={inputStyle as any}>
            {ALL_CURRENCY_CODES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </label>
          ...
          <div key={u.upload_id} style={{ borderTop: i ? `1px solid ${C.navy700}55` : 'none' }}>
            <div style={{ padding: '13px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                {amt != null && !isEditing && (
                  <div style={{ fontFamily: 'Rajdhani,sans-serif', fontWeight: 700, fontSize: 16, color: C.text }}>{fmtAmt(amt, ex.currency || 'USD')}</div>
                )}
...
function SummaryBlock({ s }: { s: BillingSummary }) {
  const { t } = useT()
  const { fmtAmt, displayCurrency } = useFmt()
  const owes = s.total_balance_due > 0
  const label = displayCurrency !== s.currency ? `${displayCurrency} (source: ${s.currency})` : s.currency
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontFamily: 'DM Mono,monospace', fontSize: 12, color: C.muted, letterSpacing: '.1em', marginBottom: 8 }}>{label}</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 12 }}>
        <GradientStat icon="billing" label={t('bill.balanceDue')} value={fmtAmt(s.total_balance_due, s.currency)} from={owes ? C.amber : C.mint} to={owes ? '#f59e0b' : C.cyan} />
        <GradientStat icon="check" label={t('bill.youPaid')} value={fmtAmt(s.total_patient_paid, s.currency)} from={C.mint} to={C.mintDk} />
        <GradientStat icon="journal" label={t('bill.charges')} value={fmtAmt(s.total_charges, s.currency)} from={C.cyan} to="#3b82f6" />
      </div>
    </div>
  )
}
...
function CoverageCard({ cov, accent }: { cov: Coverage; accent: string }) {
  ...
  return (
    <Card accent={accent} style={{ marginBottom: 16 }}>
      ...
      <Row label={t('bill.copay')} value={cov.copay_amount != null ? fmtAmt(cov.copay_amount, 'USD') : t('bill.noCopay')} />
      <Row label={t('bill.deductible')} value={fmtAmt(cov.deductible_amount, 'USD')} />
      ...
      {cov.out_of_pocket_max != null && (
        <div style={{ paddingTop: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: C.muted, marginBottom: 6 }}>
            <span>{t('bill.oop')}</span><span style={{ color: C.text, fontWeight: 600 }}>{fmtAmt(cov.out_of_pocket_met, 'USD')} / {fmtAmt(cov.out_of_pocket_max, 'USD')}</span>
          </div>
...
function MarkPaidDialog({ inv, onDone, onCancel }: { inv: Invoice; onDone: () => void; onCancel: () => void }) {
  ...
  const showHint = displayCurrency !== inv.currency && parsedAmt > 0
  const convertedHint = showHint ? fmtAmt(parsedAmt, inv.currency) : null

  async function submit() {
    if (!amount || parsedAmt <= 0) return
    setBusy(true); setErr(null)
    try {
      await markPayment(inv.invoice_id, parsedAmt, method, reference || undefined, note || undefined)
      onDone()
    } catch (e: any) {
  ...
  return (
    <div style={{ padding: '0 20px 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
      <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <span style={{ fontSize: 11.5, color: C.muted }}>{t('bill.amount')} ({inv.currency})</span>
        <input type="number" value={amount} onChange={e => setAmount(e.target.value)} style={inputStyle} />
        {convertedHint && <span style={{ fontSize: 11.5, color: C.subtle }}>≈ {convertedHint}</span>}
      </label>
      <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontFamily: 'Rajdhani,sans-serif', fontWeight: 700, fontSize: 20, color: inv.balance_due > 0 ? C.amber : C.green }}>{fmtAmt(inv.balance_due, inv.currency)}</div>
                    <div style={{ fontSize: 11.5, color: C.subtle }}>{t('bill.ofCharges', { total: fmtAmt(inv.total_charges, inv.currency) })}</div>
                  </div>
    ...
    <Card style={{ padding: 0, overflow: 'hidden' }}>
      {items.map((p, i) => (
        <div key={p.payment_id} style={{ padding: '13px 20px', borderTop: i ? `1px solid ${C.navy700}55` : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontFamily: 'Rajdhani,sans-serif', fontWeight: 700, fontSize: 18, color: C.mint, flexShrink: 0 }}>{fmtAmt(p.payment_amount, p.currency)}</div>
        </div>
      ))}
    </Card>
  )
    ...
    <Card style={{ padding: 0, overflow: 'hidden' }}>
      {items.map((r, i) => (
        <div key={r.era_id} style={{ padding: '13px 20px', borderTop: i ? `1px solid ${C.navy700}55` : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontFamily: 'Rajdhani,sans-serif', fontWeight: 700, fontSize: 18, color: C.cyan, flexShrink: 0 }}>{fmtAmt(r.total_payment, r.currency)}</div>
        </div>
      ))}
    </Card>
  )
...
export default function Billing() {
  ...
  const fxAsync = useAsync(fetchRates, [])
  const bump = () => setRefresh(x => x + 1)

  const [displayCurrency, setDisplayCurrencyState] = useState<string>(() => {
    try { return localStorage.getItem('ptc.billingCurrency') ?? 'USD' } catch { return 'USD' }
  })
  function setDisplayCurrency(c: string) {
    setDisplayCurrencyState(c)
    try { localStorage.setItem('ptc.billingCurrency', c) } catch { /* ignore */ }
  }

  const rates = fxAsync.data ?? null
  const fmtAmt: FmtAmt = (n, nativeCurrency = 'USD') => {
    if (n == null) return '—'
    if (!rates || displayCurrency === nativeCurrency) return money(n, nativeCurrency)
    return money(convert(n, nativeCurrency, displayCurrency, rates), displayCurrency)
  }

  return (
    <FxContext.Provider value={{ fmtAmt, displayCurrency }}>
      <div className="cmp-fade-up">
        <SectionHeader icon="billing" title={t('bill.title')} sub={t('bill.subtitle')} color={A.c} />

        <CurrencySelector value={displayCurrency} onChange={setDisplayCurrency} accent={A.c} />
        {displayCurrency !== 'USD' && rates && (
          <div style={{ fontSize: 11.5, color: C.subtle, marginBottom: 14, lineHeight: 1.5 }}>{t('bill.fxApprox')}</div>
        )}
